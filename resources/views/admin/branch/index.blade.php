        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center pb-4 border-b rounded-md mb-4">
            ...
            <div class="flex flex-row gap-2 mt-3 md:mt-0 w-full md:w-auto items-start sm:items-center">
                <a href="{{ route('admin.branches.create') }}"
                    ...
                    <i class="ri-add-line text-lg"></i>Create Branch
                </a>
            </div>
        </div>
            ...
            <table class="min-w-full table-auto">
                <thead class="bg-blue-600 text-white text-sm font-semibold">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right pr-8">Warkpace/Actions</th>
                    </tr>
                </thead>
                <tbody class="text-sm text-gray-700 divide-y divide-gray-200">
                    @foreach ($branches as $index => $user)
                        <tr class="hover:bg-gray-100 transition-colors cursor-pointer">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">{{ $user->institute_name }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ $user->director_name }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ $user->username }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ $user->mobile_number }}</td>
                            <td class="px-4 py-3 text-right whitespace-nowrap">
                                <div class="flex justify-end items-center gap-1">
                                    ...
                                    <a href="{{ route('admin.branches.show', $user->id) }}"
                                    ...
                                    <a href="{{ route('admin.branches.work', $user->id) }}"
                                        class="inline-flex items-center justify-center px-3 h-8 bg-yellow-500 hover:bg-yellow-600 text-white rounded shadow text-sm font-medium"
                                        title="Work">
                                        Work
                                    </a>

                                    <form action="{{ route('admin.branches.status', $user->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                            class="inline-flex items-center justify-center px-3 h-8 
                                        {{ $user->status == 'Active' ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-500 hover:bg-gray-600' }}
