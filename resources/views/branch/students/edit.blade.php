    ...
    <div class="w-full flex flex-col gap-4 mb-20">
        <div class="flex flex-col bg-white shadow rounded md:p-6 p-4 md:gap-1 gap-3">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold text-gray-800 mb-2">Edit Student</h2>
            ...
            <div class="flex justify-between items-center text-gray-600 text-sm">
                <p>
                    <a href="{{ route('branch.dashboard') }}" class="text-blue-600 hover:underline">Home</a> / Student / Edit
                </p>
                <a href="{{ route('branch.students.index') }}"
        ...
        <div class="w-full bg-white rounded shadow px-6 py-6">
            <form action="{{ route('branch.students.update', $student->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $student->name) }}"
                        ...
                        class="w-full text-gray-700 rounded-md border border-gray-300 px-3 py-2 text-sm sm:text-base outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="institute_name"
                        value="{{ old('institute_name', $student->institute_name) }}" placeholder="Institute Name"
                        class="w-full text-gray-700 rounded-md border border-gray-300 px-3 py-2 text-sm sm:text-base outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="roll" value="{{ old('roll', $student->roll) }}" placeholder="Roll Number"
                        class="w-full text-gray-700 rounded-md border border-gray-300 px-3 py-2 text-sm sm:text-base outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="registration_no"
                        value="{{ old('registration_no', $student->registration_no) }}" placeholder="Registration No"
                        class="w-full text-gray-700 rounded-md border border-gray-300 px-3 py-2 text-sm sm:text-base outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200">
                </div>
                ...
                <div>
                    ...
                    <select name="student_type"
                        class="w-full text-gray-700 rounded-md border border-gray-300 px-3 py-2.5 text-sm sm:text-base outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200">
                        <option value="">Select Type</option>
                        <option value="Regular"
                            {{ old('student_type', $student->student_type) == 'Regular' ? 'selected' : '' }}>Regular
                        </option>
                        <option value="Irregular"
                            {{ old('student_type', $student->student_type) == 'Irregular' ? 'selected' : '' }}>Irregular
                        </option>
                    </select>
                </div>
                    ...
                    <select name="course_name"
                        ...
                        @foreach ($courses as $course)
                            <option value="{{ $course->name }}"
                                {{ old('course_name', $student->course_name) == $course->name ? 'selected' : '' }}>
                                {{ $course->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="course_duration"
                        ...
                        @for ($i = 1; $i <= 12; $i++)
                            <option value="{{ $i }} Month"
                                {{ old('course_duration', $student->course_duration) == $i . ' Month' ? 'selected' : '' }}>
                                {{ $i }} Month{{ $i > 1 ? 's' : '' }}
                            </option>
                        @endfor
                        ...
                        @for ($i = 1; $i <= 5; $i++)
                            <option value="{{ $i }} Year"
                                {{ old('course_duration', $student->course_duration) == $i . ' Year' ? 'selected' : '' }}>
                                {{ $i }} Year{{ $i > 1 ? 's' : '' }}
                            </option>
                        @endfor
                    </select>
                ...
                <div class="col-span-2">
                    ...
                    @if ($student->profile_photo)
                        <div class="mt-3">
                            ...
                            <img src="{{ asset($student->profile_photo) }}" alt="Profile"
